...
(async () => {
  ...
  await page.screenshot({ path: 'screenshots/homepage-results.png', fullPage: true });
  console.log('2. Captured: homepage-results.png (with test results + quality score)');
  ...
  if (await shareButton.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    await page.screenshot({ path: 'screenshots/share-modal.png', fullPage: false });
  ...
  const dateFilter = page.locator('button:has-text("30d"), button:has-text("7d")').first();
  if (await dateFilter.isVisible({ timeout: 3000 }).catch(() => false)) {
    await dateFilter.click();
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'screenshots/history-filter.png', fullPage: false });
    console.log('5. Captured: history-filter.png (date filter dropdown)');
    await page.keyboard.press('Escape');
  }

  // ===== 6. SETTINGS PAGE =====
  await page.goto('https://speed-test-app-nu.vercel.app/en/settings', { waitUntil: 'networkidle' });
  ...
  const themeButtons = page.locator('button:has-text("Dark"), button:has-text("Light"), button:has-text("Auto")');
  const themeCount = await themeButtons.count();
  if (themeCount > 1) {
    await themeButtons.nth(1).click(); // Click second theme option
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'screenshots/settings-theme.png', fullPage: true });
    console.log('7. Captured: settings-theme.png (theme toggle)');
  }

  // ===== 8. SERVERS PAGE =====
  await page.goto('https://speed-test-app-nu.vercel.app/en/servers', { waitUntil: 'networkidle' });
  await page.waitForTimeout(1500);
  await page.screenshot({ path: 'screenshots/servers-page.png', fullPage: true });
  ...
  const mobileStartButton = page.locator('button:has-text("INITIALIZE")').first();
  ...
  await page.goto('https://speed-test-app-nu.vercel.app/en/about', { waitUntil: 'networkidle' });
  await page.waitForTimeout(1500);
  ...
  await page.goto('https://speed-test-app-nu.vercel.app/en', { waitUntil: 'networkidle' });
  await page.waitForTimeout(1500);
